...
import 'package:mathgame/src/ui/common/common_level_up_dialog_view.dart';
    ...
    switch (provider.dialogType) {
      case DialogType.over:
        showDialog<bool>(
          ...
          if (value != null && value) {
            AdManager.instance.showRestartRewardedAd(
              onRewardEarned: () {
      ...
      case DialogType.levelUp:
        provider.dialogType = DialogType.non;
        showDialog<bool>(
          context: context,
          builder: (newContext) => CommonAlertDialog(
            child: CommonLevelUpDialogView(
              gameCategoryType: widget.gameCategoryType,
              score: context.read<T>().currentScore,
              questionCount: context.read<T>().index,
            ),
          ),
          barrierDismissible: false,
        ).then((value) {
